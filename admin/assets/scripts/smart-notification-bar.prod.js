...
jQuery(document).ready(function () {
  ...
  jQuery(`.selectric`).selectric({
    ...
    onChange: function (element) {
      const currentSelect = element.id ? element.id : null; // updata preview
...
($ => {
  class epSmartNotificationbar {
    constructor() {
      _defineProperty(this, "notificationBarPreviewElements", {
      ...
      _defineProperty(this, "colorPreviewMove", (item, color) => {
      ...
      _defineProperty(this, "updatePreview", () => {
        // update content
        $(`#smart-notification-bar__editor-preview .preview__body`).html($(`#content`).val());
        let height = $(`#height`).val(); // update height
        ...
        $(`#smart-notification-bar__editor-preview .preview__body`).attr("class", `preview__body ${$(`#font_family`).val()}`); // update background color, background image
        ...
        $(`#smart-notification-bar__editor-preview a`).attr(`class`, `cta__content ep_smart_notification_bar_cta_style_${$("input[name='cta_style']:checked").val()} ep_smart_notification_bar_cta_animation_${$("input[name='cta_animation']:checked").val()} ${$(`#cta_font_family`).val()}`).html($("#cta_content").val()).attr(`href`, $("#cta_url").val()).css({
          color: $(`#cta_color`).val(),
      ...
      _defineProperty(this, "toggleDateSections", action => {
      ...
      _defineProperty(this, "toggleCtaSettingsSections", () => {
      ...
      _defineProperty(this, "toggleCloseSettingsSections", () => {
      ...
      _defineProperty(this, "toggleDisplayRuleSettings", () => {
        let selectedOption = $(`input[name="display_rule"]:checked`).val();

        if (selectedOption == "defined_pages") {
          // show defined pages settings
          $(`.display-rule-defined-pages-section`).removeClass(`hidden`); // hide excluded pages settings

          $(`.display-rule-excluded-pages-section`).addClass(`hidden`);
        } else if (selectedOption == "excluded_pages") {
          // show excluded pages settings
          $(`.display-rule-excluded-pages-section`).removeClass(`hidden`); // hide defined pages settings

          $(`.display-rule-defined-pages-section`).addClass(`hidden`);
        } else {
          // hide defined pages settings
          $(`.display-rule-defined-pages-section`).addClass(`hidden`); // hide excluded pages settings

          $(`.display-rule-excluded-pages-section`).addClass(`hidden`);
        }
      });

      _defineProperty(this, "toggleVisitorSourceRules", () => {
      ...
      _defineProperty(this, "toggleSpecificParameters", () => {
      ...
      _defineProperty(this, "checkValue", element => {
        if (!element.value) {
      ...
      _defineProperty(this, "saveNotificationBarData", () => {
        ...
        $(`.ep-plugin-save-smart-notification-bar`).each((i, button) => {
          // disable button
          $(button).attr(`disabled`, true); // show loading icon
        ...
        $.post(ajaxurl, data, response => {
          ...
          $(`.ep-plugin-save-smart-notification-bar`).each((i, button) => {
            // enable button
            $(button).attr(`disabled`, false); // show loading icon
      ...
      _defineProperty(this, "deleteNotificationbar", event => {
      ...
      _defineProperty(this, "duplicateNotificationbar", event => {
        event.target.querySelector("i").classList.remove("hidden");
        event.target.disabled = true;
      ...
      _defineProperty(this, "toggleDeleteConfirmModal", () => {
      ...
      _defineProperty(this, "toggleHiddenAfterClosed", () => {
      ...
      _defineProperty(this, "uploadBackgroundImage", () => {
        let backgroundImageUploader;
        let attachment;
        backgroundImageUploader ? backgroundImageUploader.open() : ((backgroundImageUploader = wp.media.frames.backgroundImageUploader = wp.media({
          ...
          attachment = backgroundImageUploader.state().get("selection").first().toJSON(); // set background image field value

          $(`#background_image`).val(attachment.url); // display background image preview

          $(`#background-image-preview img`).attr(`src`, attachment.url);
          $(`#background-image-preview`).removeClass(`hidden`); // update preview

          $(`#background_image`).trigger("change");
        }), backgroundImageUploader.open());
      });

      _defineProperty(this, "removeBackgroundImage", () => {
        // set background image field value
        $(`#background_image`).val(""); // hide background image preview

        $(`#background-image-preview img`).attr(`src`, "");
        $(`#background-image-preview`).addClass(`hidden`); // update preview

        $(`#background_image`).trigger("change");
      });

      _defineProperty(this, "toggleCTAanimationPreview", e => {
      ...
      _defineProperty(this, "toggleFormSectionBody", e => {
      ...
      _defineProperty(this, "notificationBarTemplate", "");

      _defineProperty(this, "toggleTemplatePreview", (e, action) => {
        if (action == "display") {
      ...
      _defineProperty(this, "useSelectedTemplate", () => {
        const templates = {
          light: {
            font_color: "#333333",
      ...
      _defineProperty(this, "init", () => {
        ...
        $(`#content`).on("keyup", this.updatePreview); // on height change

        $(`#height`).on("change", this.updatePreview);
        $(`#height`).on("keyup", this.updatePreview); // on font color change

        $(`#font_color`).on("change", this.updatePreview); // on font family change

        $(`#font_family`).on("change", this.updatePreview); // on font size change

        $(`#font_size`).on("keyup", this.updatePreview);
        $(`#font_size`).on("change", this.updatePreview); // on background color change

        $(`#background_color`).on("change", this.updatePreview); // on background image change

        $(`#background_image`).on("change", this.updatePreview); // on display cta change

        $(`input[name="display_cta"]`).on("change", this.updatePreview); // on cta style change

        $(`input[name="cta_style"]`).on("change", this.updatePreview); // on cta animation change

        $(`input[name="cta_animation"]`).on("change", this.updatePreview); // on cta content change

        $(`#cta_content`).on("keyup", this.updatePreview); // on cta url change

        $(`#cta_url`).on("keyup", this.updatePreview); // on cta font color change

        $(`#cta_color`).on("change", this.updatePreview); // on cta background color change

        $(`#cta_background`).on("change", this.updatePreview); // on cta font size change

        $(`#cta_font_size`).on("keyup", this.updatePreview);
        $(`#cta_font_size`).on("change", this.updatePreview); // on cta font family change

        $(`#cta_font_family`).on("change", this.updatePreview); // on display close button change

        $(`input[name="display_close_button"]`).on("change", this.updatePreview); // on close button color change
